...
export default function CheckinPage() {
  ...
  useEffect(() => {
    fetch('/api/auth/me').then((r) => r.json()).then((data) => {
      setMe(data.member);
      setStep(data.member ? 'confirm' : 'loading');
    });
  }, []);
  ...
  if (step === 'loading') {
    return <p className="text-sm text-ink/50">Please <a href="/login" className="font-bold text-plum">log in</a> to check in.</p>;
  }
  ...
  if (step === 'confirm' && me) {
    return (
      <div className="mx-auto max-w-sm">
        ...
        <Button onClick={handleCheckIn} className="w-full">Confirm & check in</Button>
      </div>
    );
  }
